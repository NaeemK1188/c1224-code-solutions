...
async function throwSeveral(): Promise<void> {
  ...
  const msg1 = await read('foo2', false); // it wont continue te execution because its false
...
// using async/await with try/catch
try {
  // every function is dependent on each other, so if one await is false, the try will skip to catch
  await throwOnce();
  await throwSeveral();
  await throwChained();
} catch (error) {
